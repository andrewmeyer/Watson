...
# aggregate
complete -f -c watson -n '__fish_watson_needs_sub' -a aggregate -d "Display report of time per project by day"
complete -f -c watson -n '__fish_watson_using_command aggregate' -s c -l current -d "include the running frame"complete -f -c watson -n '__fish_watson_using_command aggregate' -s C -l no-current -d "exclude the running frame (default)"
complete -f -c watson -n '__fish_watson_using_command aggregate' -s f -l from -d "Start date for aggregate"
complete -f -c watson -n '__fish_watson_using_command aggregate' -s t -l to -d "End date for aggregate"
